    ...
    def test_page_open_detail(self):
        class MockCacheValue(mock.MagicMock):
            def get(*args, **kwargs):
                dummy = 0
                if args:
                    if args[0] == "feedback_list_1":
                        with open("mainapp/fixtures/005_feedback_list_1.bin", "rb") as inpf:
        ...
        with mock.patch(
            ...
            # doesn't work because of lesson cache
            # mocked_cache.return_value = pickle.load(inpf)
            mocked_cache.side_effect = MockCacheValue.get
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def setUp(self):
        super().setUp()
        # ff_options = {'profile-root':'settings.SELENIUM_DRIVER_PATH_FF'}
        # binary = FirefoxBinary('~/Desktop/djangoBasics/GB_Python_Django01/var/selenium')
        # binary.add_command_line_options('--profile-root=~/tmp')
        # driver = Webdriver.Firefox(firefox_binary=binary)
        # self.selenium = WebDriver(firefox_binary=binary)
        # os.environ["TMPDIR"] = "~/tmp"
        # profile = FirefoxProfile()
        # profile.profile_dir = '~/tmp'
